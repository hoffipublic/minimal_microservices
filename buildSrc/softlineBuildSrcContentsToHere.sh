...
files=( build.gradle.kts src/main/kotlin/c/commons.kt src/main/kotlin/secure/security.kt src/main/kotlin/spring/springProperties.kt src/main/kotlin/v/dependencies.kt )

# latest versions in https://github.com/hoffipublic/gradle_dependencies
for f in ${files[@]}; do
    >&2 echo ln -s $(fackDirs $f)/$centralGradleDependenciesDir/$f ./$f
